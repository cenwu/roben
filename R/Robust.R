...
Robust <- function(xx, y, CLC, s, L, max.steps, hatAlpha, hatBeta, sparse, structure, hyper, debugging){
  ...
  if(sparse){
    fit=switch (structure,
                ...
                "group" = BRGL_SS(xx, y, CLC, s, L, max.steps, hatAlpha, hatBeta, hatTau, hatV, hatSg, invSigAlpha0, hatPi, hatEtaSq, xi1, xi2, r, a, b, sh1, sh0, progress),
                "individual" = BRL_SS(xx, y, CLC, max.steps, hatAlpha, c(hatBeta), hatTau, hatV, hatSg=rep(1, s*L), invSigAlpha0, hatPi, hatEtaSq, xi1, xi2, r, a, b, sh1, sh0, progress)
    )
  }else{
    hatBeta = hatBeta + 10^-5
    fit=switch (structure,
                "sparsegroup" = BRSGL(xx, y, CLC, s, L, max.steps, hatAlpha, hatBeta, hatTau, hatV, hatSg, hatGamma, invSigAlpha0, hatEta1Sq, hatEta2Sq, xi1, xi2, s1=1, s2=1, r1, r2, a, b, progress),
